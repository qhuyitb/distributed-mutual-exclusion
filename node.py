...
class Node:
    ...
    def _enter_cs(self):
        with self.lock:
            self._use_shared_resource()

        with self.lock:
            self.logger.log("<<< EXIT CS", self.clock.get_time())
            self._exit_cs()

    def _use_shared_resource(self):
        """Sử dụng tài nguyên chia sẻ (shared.txt)"""
        try:
            with self.file_lock:
                ...
                # Mô phỏng công việc trong CS
                time.sleep(CS_DURATION)
                
                # Ghi thêm dòng log từ node hiện tại
                timestamp = datetime.now().strftime("%H:%M:%S.%f")[:-3]
                entry = f"\n[{timestamp}] Node {self.node_id} accessed resource (Lamport ts={self.request_timestamp}, seq={self.seq})"
                
                with open(self.SHARED_FILE, 'a') as f:
                    f.write(entry)
                
                self.logger.log(f"Wrote to shared resource: {entry.strip()}", self.clock.get_time())
        except Exception as e:
            self.logger.log(f"ERROR using shared resource: {e}", self.clock.get_time())
        with self.lock:
            self.logger.log("<<< EXIT CS", self.clock.get_time())
            self._exit_cs()
