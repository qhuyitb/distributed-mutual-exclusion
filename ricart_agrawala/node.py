...
class Node:
    def __init__(self, node_id, host, port, peers, adapter=None):
        self.node_id = node_id
        self.host = host
        self.port = port
        self.peers = peers
    ...
    def _on_message(self, msg: Message):
        ...
        self.logger.log(
            f"RECEIVE {msg.msg_type} from Node {msg.sender_id} (ts={msg.timestamp})",
            self.clock.get_time()
        )
        ...
        with self.lock:
            ...
            if msg.msg_type == "REQUEST":
                self._log_message_received("REQUEST", msg.sender_id, {"timestamp": msg.timestamp})
                self._handle_request(msg)
            elif msg.msg_type == "REPLY":
                self._log_message_received("REPLY", msg.sender_id, {"timestamp": msg.timestamp})
    ...
    def _handle_request(self, msg: Message):
        ...
        self.logger.log(
            f"Queue: {[f'({r.timestamp},{r.node_id})' for r in self.request_queue]}",
            self.clock.get_time()
        )
    ...
    def _handle_reply(self, msg: Message):
        ...
        self.reply_count += 1
        self.logger.log(
            f"RECEIVE REPLY from Node {msg.sender_id} (count={self.reply_count})",
            self.clock.get_time()
        )
    ...
    def _send_request(self):
        ...
        for peer_id, (p_host, p_port) in self.peers.items():
            ...
            self._log_message_sent("REQUEST", peer_id, {"timestamp": self.request_timestamp})
            threading.Thread(
                target=self.network.send_message,
                args=(p_host, p_port, msg),
                daemon=True
            ).start()
    ...
    def _send_reply(self, target_id):
        ...
        self._log_message_sent("REPLY", target_id, {"timestamp": reply_ts})

        threading.Thread(
            target=self.network.send_message,
            args=(target[0], target[1], msg),
            daemon=True
        ).start()
    ...
    def _enter_cs(self):
        ...
        with self.lock:
            ...
            self._log_enter_cs()
            self.logger.log(
                f">>> ENTER CS (request timestamp={self.request_timestamp})",
                self.clock.get_time()
            )
        ...
        try:
            ...
            self.logger.log(
                f"Updated shared.txt: Node {self.node_id} - {message}",
                self.clock.get_time()
            )
    ...
    def _log_request_cs(self):
        if self.adapter:
            self.adapter.log_request_cs(self.node_id)

    def _log_enter_cs(self):
        if self.adapter:
            self.adapter.log_enter_cs(self.node_id)

    def _log_exit_cs(self):
        if self.adapter:
            self.adapter.log_exit_cs(self.node_id)

    def _log_message_sent(self, msg_type, receiver_id, data=None):
        if self.adapter:
            self.adapter.log_message_sent(msg_type, self.node_id, receiver_id, data or {})

    def _log_message_received(self, msg_type, sender_id, data=None):
        if self.adapter:
            self.adapter.log_message_received(msg_type, sender_id, self.node_id, data or {})